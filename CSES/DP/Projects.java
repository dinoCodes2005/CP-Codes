...
public class Projects {
    ...
    public void solveTestCase() throws Exception {
        ...
        int n = in.i();
        int a[][] = new int[n][3];
        Map<Integer,List<int[]>> ends_at = new HashMap<>();
        ...
        for(int i=0;i<lst.size();i++){
            compressed.putIfAbsent(lst.get(i),day++);
        }
        for(int row[]:a){
            ends_at.computeIfAbsent(compressed.get(row[1]), k->new ArrayList<>()).add(new int[]{compressed.get(row[0]),row[2]});
        }
 
        long dp[] = new long[day];
        for(int i=1;i<day;i++){
            dp[i] = dp[i-1];
            List<int[]> ends = ends_at.get(i);
