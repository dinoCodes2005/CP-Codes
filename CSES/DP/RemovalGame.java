...
public class RemovalGame {
    ...
    public long f(int l,int r,int turn){
        if(l == r){
            return turn == 0 ? x[l] : -x[r];
        }
        if(dp[l][r][turn] != null) return dp[l][r][turn];

        if(turn == 0) return dp[l][r][turn] = Math.max(x[l]+f(l+1,r,1),x[r]+f(l,r-1,1));
        else return dp[l][r][turn] = Math.min(-x[l]+f(l+1,r,0),-x[r]+f(l,r-1,0));
    }
