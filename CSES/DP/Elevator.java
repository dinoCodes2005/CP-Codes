...
public class Elevator {
    ...
    public void solveTestCase() throws Exception {
        ...
        for(int mask=0;mask<(1<<n);mask++){
            for(int i=0;i<n;i++){
                int new_guy = 1<<(n-i-1);
                if((mask & new_guy) == 0) continue;
                int before = mask ^ new_guy;
                long before_weight = dp[before][1];
                if(before_weight + w[i] <= x){
                    dp[mask] = min(dp[mask],new long[]{dp[before][0] ,before_weight+w[i]});
                }else{
                    dp[mask] = min(dp[mask],new long[]{dp[before][0]+1,w[i]});
                }
            }
        }

        // for(long row[]:dp){
        //     for(long val:row) out.p(val+" ");
        //     out.pl();
        // }
        out.pl(dp[(1<<n)-1][0]);
    }
