...
public class PermutationRounds {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // }
        PermutationRounds obj = new PermutationRounds();
        obj.solveTestCase();
        out.flush();
        out.close();
    }
    ...
    public void solveTestCase() throws Exception {
        ...
        for(int val:dt){
            ...
            for(int j=0;j<primes.size();j++){
                int cnt = 0;
                int p = primes.get(j);
                if(p*1l*p > v) break inner; 
                while(v % p == 0){
                    v /= p;
                    cnt++;
                }
                power.put(p,Math.max(power.getOrDefault(p,0),cnt));
            }
            if (v > 1) {
                power.put(v, Math.max(power.getOrDefault(v, 0), 1));
            }
        }

        long res = 1l;
        for(int key:power.keySet()){
            long now = modPow(1l*key, 1l*power.get(key), mod);
            res = (res * now) % mod;
        }
