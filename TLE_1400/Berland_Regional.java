...
public class Berland_Regional {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int t = in.i();
        Berland_Regional obj = new Berland_Regional();
    ...
    public void solveTestCase() throws IOException {
        ...
        int s[] = read(n);
        for(int i=0;i<n;i++){
        	m.computeIfAbsent(a[i],k->new ArrayList<>()).add(new int[]{i,s[i]});
        }
        Map<Integer,List<Integer>> p = new HashMap<>();
        for(int key:m.keySet()){
        	List<int[]> now = m.get(key);
        	List<Integer> pref = new ArrayList<>();
        	pref.add(now.get(0)[1]);
        	for(int i=1;i<now.size();i++) pref.add(pref.get(i-1) + now.get(i)[1]);
        	p.put(key,pref);
        }
        for(int k=1;k<=n;k++){
        	long ans = 0;
