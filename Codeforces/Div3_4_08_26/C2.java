...
public class C2 {
    ...
    public void solveTestCase() throws Exception {
        ...
        List<Integer> o_a = new ArrayList<>();
        List<Integer> o_b = new ArrayList<>();
        for(int i=1;i<n;i+=2){
            if(a.c(i) == '1') o_a.add(i);
            if(b.c(i) == '1') o_b.add(i);
        }

        if(o_a.size() != o_b.size()){
            out.pl(-1);
            return;
        }

        List<Integer> e_a = new ArrayList<>();
        List<Integer> e_b = new ArrayList<>();
        for(int i=0;i<n;i+=2){
            if(a.c(i) == '1') e_a.add(i);
            if(b.c(i) == '1') e_b.add(i);
        }
        ...
        long x = 0;
        for(int k=0;k<o_a.size();k++){
            x += 1l*Math.abs(o_a.get(k) - o_b.get(k));
        }
        x /= 2l;

        long y = 0;
        for(int k=0;k<e_a.size();k++){
            y += 1l*Math.abs(e_a.get(k) - e_b.get(k));
        }
        y /= 2l;
        long res = x+y;
        out.pl(res);
