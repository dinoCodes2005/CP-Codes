...
public class D {
    ...
    public void solveTestCase() throws Exception {
        ...
        Map<Long,Long> b_to_a = new HashMap<>();
        long extra = 0;
        long last_count = 0;
        long last_key = 0;
        long last_number = -1;
        ...
        for(long key:map.keySet()){
            if(key == 0){
                last_count = map.get(key);
                last_key = key;
                continue;
            }else{
                long d = key - extra;
                if(d <= 0 || (d % last_count != 0)){
                    out.pl(-1);
                    return;
                }
                long now = d / last_count; 
                if(now <= last_number){
                    out.pl(-1);
                    return;
                }
                last_number = now;
                b_to_a.put(last_key,last_number);
                extra += last_number * last_count;
                last_count = map.get(key);
                last_key = key;
            }
        }
        long largest = map.lastKey();
        if(map.size() != 1){
            b_to_a.put(largest,last_number+1);
        }
        for(int i=0;i<n;i++){
            out.p(b_to_a.get(b[i])+" ");
        } 
        out.pl();
    }
