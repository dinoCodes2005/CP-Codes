...
public class B {
    ...
    Long dp[][][];
    public long f(int n,char a[],int i,int last_sum,int last_val){
        if(i == n) return 1l;
        if(dp[i][last_sum][last_val] != null) return dp[i][last_sum][last_val];
        long ans = 0;
        char c = a[i];
        if(c == '?'){
            for(int now=0;now<2;now++){
                if(now + last_val != last_sum) {
                    ans = (ans + f(n,a,i+1,now+last_val,now)) % mod;
                }
            }
        }else if(c-'0' + last_val == last_sum){
            return 0l;
        }else{
            ans = f(n,a,i+1,(c-'0')+last_val,(c-'0'));
        }

        return dp[i][last_sum][last_val] = ans;
    }

    public void solveTestCase() throws Exception {
        // write code
        int n = in.i();
        char a[] = in.nl().toCharArray();
        dp = new Long[n+1][3][2];
        long ans = 0;
        int f = a[0] - '0';
        int s = a[1] - '0';
        if(a[0] == '?' && a[1] == '?'){
            for(int i=0;i<2;i++){
                for(int j=0;j<2;j++){
                    ans = (ans + f(n,a,2,i+j,j)) % mod;
                }
            }
        }else if(a[0] != '?' && a[1] == '?'){
            for(int j=0;j<2;j++){
                ans = (ans + f(n,a,2,f+j,j)) % mod;
            }
        }else if(a[0] == '?' && a[1] != '?'){
            for(int i=0;i<2;i++){
                ans = (ans + f(n,a,2,s+i,s)) % mod;
            }
        }else{
            ans = (ans + f(n,a,2,f+s,s)) % mod;
        }
        out.pl(ans);
    }
